...
{
    public class OperationsService<T>
    {
        private static DbConnectionClass _context;
        private static IMapper _mapper;

        public OperationsService(DbConnectionClass context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public static List<T> GetList()
        {
            ...
            var GetList = new List<T>();
            foreach (var item in DataList)
            {
                var VM = _mapper.Map<T>(item);
                GetList.Add(VM);
            }
            return GetList;
        }

        public static bool Add(T collection)
        {
            try
            {
                if (collection != null)
                {
                    var MapData = _mapper.Map<T>(collection);
        ...
        public static async Task<bool> Update(T collection)
        {
            try
            {
                if (collection != null)
        ...
        public static T Find(string id)
        {
            ...
            var VM = _mapper.Map<T>(FoundRecord);
